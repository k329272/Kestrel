...
We rotate the frame through several angles and query the model once per angle.
Each reading gives us an x-position measured along a different direction,
effectively a caliper measurement of the object from that angle. Projected
back into the original frame and combined, these readings trace out a polygon
that hugs the object much more tightly than a single axis-aligned guess.
...
very information the model is trying to measure. In other words: crop
dimensions are chosen per-rotation to protect x, not y.
"""
...
def estimate_polygon(bbox_predict_fn, padded, pad_offset, target_w, target_h,
                      avg_center_y, avg_height, num_rotations=4):
